...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        ...
        const isDirectoryExist = fs.existsSync(`${defaultPath}/${childDirectory}`)

        if (!isDirectoryExist) fs.mkdirSync(`${defaultPath}/${childDirectory}`, { recursive: true })
...
const fileFilter = (req: any, file: any, cb: any) => {
    const formatAccepted = ["png", "jpg", "jpeg", "svg", "webp",]
    let originalNameSplit = file.originalname.split(".")
